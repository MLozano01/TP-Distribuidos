...
def get_join_strategy(strategy_name: str, replica_id: int, replicas_count: int, state_manager):
    """
    Factory function to get a join strategy instance.
    """
    if strategy_name == "RATINGS":
        return RatingsJoinStrategy(replica_id, replicas_count, state_manager)
    elif strategy_name == "CREDITS":
        return CreditsJoinStrategy(replica_id, replicas_count, state_manager)
    else:
